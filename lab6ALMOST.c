...
int main (int argc, char *argv[])
{
   ...
   //"installing" the sighandler and linking it to the shutdown() function
   ...
   int lCtrlRetVal = semctl(semId, 0, SETVAL, IPC_SET);
   ...
   waitbuf.sem_num = 0;
   waitbuf.sem_op = -1;  //reduce the semaphore count to lock
   waitbuf.sem_flg = 0;  //wait till we get lock on semaphore

   struct sembuf signalbuf;
   signalbuf.sem_num = 0;
   signalbuf.sem_op = 1;    //increase the semaphore count to release
   signalbuf.sem_flg = 0;   //wait till we get lock on semaphore
