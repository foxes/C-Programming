...
int main (int argc, char* argv[]) {
    ...
    //creating pointer instance of sharedMemoryData
    ...
    //keeps accepting the input until detects sharedMemoryPointer shares "quit" input from writer
    while(input.compare("quit") != 0) {
        if (sharedMemoryPointer->newMessage == true && sharedMemoryPointer->oldMessage < 1) {
            input = string(sharedMemoryPointer->user_input); 
            cout << input << endl;
            //checking for new messages and setting boolean to decide if it should print data from sharedMemory
            sharedMemoryPointer->oldMessage++;
            if (sharedMemoryPointer->oldMessage > 1) 
                sharedMemoryPointer->newMessage = false;
        }
    }
